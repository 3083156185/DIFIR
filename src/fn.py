...
LINE_COLORS = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0],
...
def draw_single(frame, pts, joint_format='coco'):
    ...
    circle_size = max(2, int(np.sqrt(np.max(np.max(pts, axis=0) - np.min(pts, axis=0)) // 16)))
    for n in range(pts.shape[0]):
        if pts[n, 2] <= 0.05:
            continue
        cor_x, cor_y = int(pts[n, 0]), int(pts[n, 1])
        part_line[n] = (cor_x, cor_y)
        cv2.circle(frame, (cor_x, cor_y), circle_size, p_color[n], -1,lineType= cv2.LINE_AA)
    for i, (start_p, end_p) in enumerate(l_pair):
        if start_p in part_line and end_p in part_line:
            start_xy = part_line[start_p]
            end_xy = part_line[end_p]
            cv2.line(frame, start_xy, end_xy, line_color[i], int(1*(pts[start_p, 2] + pts[end_p, 2]) + 1),lineType=cv2.LINE_AA)
    return frame
