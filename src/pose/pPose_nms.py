...
def pose_nms(bboxes, track_ids, pose_preds, pose_scores):
    """
    ...
    while human_scores.shape[0] != 0:
        ...
        track_ids = np.delete(track_ids, delete_ids, axis=0)

    assert len(merge_ids) == len(pick)
    bboxs_pick = ori_bboxes[pick]
    preds_pick = ori_pose_preds[pick]
    scores_pick = ori_pose_scores[pick]
    track_ids_pick = ori_track_ids[pick]
...
def pose_nms_new(bboxes,  pose_preds, pose_scores):
    """
    Parametric Pose NMS algorithm
    bboxes:         bbox locations list (n, 4)
    bbox_scores:    bbox scores list (n,)
